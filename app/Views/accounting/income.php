            ...
            <div class="card-body">
                <table class="table table-hover align-middle">
                    ...
                    <?php foreach ($entries as $entry): ?>
                        <tr>
                            <td><?= htmlspecialchars($entry['income_date']) ?></td>
                            <td><?= htmlspecialchars($entry['category']) ?></td>
                            <td><?= htmlspecialchars($entry['account_name']) ?></td>
                            <td><?= htmlspecialchars($entry['description'] ?? '') ?></td>
                            <td class="text-end text-success fw-bold"><?= number_format((float) $entry['amount'], 2) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (empty($entries)): ?>
                        <tr><td colspan="5" class="text-center text-muted py-4">No income recorded yet.</td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="card-body">
                ...
                <form method="post" action="/accounting/income">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="category" class="form-control" placeholder="e.g. Hall Rental" required>
                    </div>
                    <div class="mb-3">
